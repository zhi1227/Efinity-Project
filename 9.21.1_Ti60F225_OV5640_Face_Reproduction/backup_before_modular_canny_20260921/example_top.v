...
(
	...
	//----------------------------------------------------------------
	// 5. 双阈值（逐像素，无窗口）
	//	调参起点：TH_HIGH=160 / TH_LOW=80（2:1）。实拍时按效果调。
	//----------------------------------------------------------------
	wire [1:0]  th_cls;
	wire        th_de;
	wire [10:0] th_x;
	...
	canny_threshold #(.TH_HIGH(12'd240), .TH_LOW(12'd120), .AWIDTH(11)) u_canny_threshold
	(
		.clk		(clk_pixel ),
		.rst_n		(rstn_pixel),
		.valid_i	(nms_de    ),
	...
	always @(posedge clk_pixel or negedge rstn_pixel) begin
		...
		else begin hys_x_d <= hys_x; hys_y_d <= hys_y; end
	end
	...
	always @(posedge clk_pixel or negedge rstn_pixel) begin
		if (!rstn_pixel) begin
			...
			canny_rgb_out <= canny_edge ? 24'hFFFFFF : 24'h000000;
