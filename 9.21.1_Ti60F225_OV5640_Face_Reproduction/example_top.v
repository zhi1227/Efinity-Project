...
(
	...
	////////////////////////////////////////////////////////////////
	// Modular color-face analysis + median-prefiltered Canny.
	// DISPLAY_MODE: 0=color with candidate contours/boxes; 1=Canny with boxes.
	localparam DISPLAY_MODE = 1;
	localparam CANNY_TH_LOW = 24;
	localparam CANNY_TH_HIGH = 64;
	wire [23:0] face_overlay_rgb;
	wire face_overlay_de, face_overlay_vs, face_overlay_hs;
	wire [3:0] detected_face_candidates;
	wire face_processing_overrun;
	face_canny_video #(
		.WIDTH(1280), .HEIGHT(720), .H_TOTAL(1650),
		.DISPLAY_MODE(DISPLAY_MODE),
		.TH_LOW(CANNY_TH_LOW), .TH_HIGH(CANNY_TH_HIGH)
	) u_face_canny_video (
		.clk(clk_pixel), .rst_n(~rst_pixel),
		.rgb_i({lcd_red,lcd_green,lcd_blue}),
		.de_i(lcd_de), .vs_i(lcd_vs), .hs_i(lcd_hs),
		.rgb_o(face_overlay_rgb), .de_o(face_overlay_de),
		.vs_o(face_overlay_vs), .hs_o(face_overlay_hs),
		.face_count(detected_face_candidates), .face_overrun(face_processing_overrun)
	);
