...
module tb_face_grid_regions;
 ...
 for(yy=0;yy<48;yy=yy+1) begin
   for(xx=0;xx<64;xx=xx+1) begin
     ...
     case(m)
       ...
       1:skin=0;
       2:skin=1;
       4:skin=(xx>=8 && xx<12 && yy>=8 && yy<12);
       5:skin=(xx>=8 && xx<56 && yy>=8 && yy<16);
       6:skin=(xx>=8 && xx<16 && yy>=8 && yy<40);
 ...
 send_frame(4);if(count!=0)$fatal(1,"small candidate not rejected");
 send_frame(5);if(count!=0)$fatal(1,"wide candidate not rejected");
 send_frame(6);if(count!=0)$fatal(1,"narrow candidate not rejected");
 $display("PASS grid: small/wide/narrow rejected; two faces, empty-frame clear, full-bank BFS, border and diagonal connectivity");
 $finish;
 end
