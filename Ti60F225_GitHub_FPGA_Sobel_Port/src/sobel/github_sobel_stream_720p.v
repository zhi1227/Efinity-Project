...
module github_sobel_stream_720p #(
    ...
    // Intensity-to-neon mapping in the existing output stage: no sync skew.
    reg [23:0] edge_color;
    always @* begin
        if (!cyber_mode) edge_color = {edge_pixel, edge_pixel, edge_pixel};
        else if (edge_pixel < 8'd24) edge_color = 24'h000000;
        else if (edge_pixel < 8'd64)
            edge_color = {8'd0, edge_pixel, edge_pixel[5:0], 2'b00};
        else if (edge_pixel < 8'd128) edge_color = 24'h00ffff;
        else if (edge_pixel < 8'd192) edge_color = {edge_pixel, 8'h20, 8'hff};
        else edge_color = 24'hff40e0;
    end
