...
(
	...
	input             key_mode_n,            // Core-board K1, active low
	...
	////////////////////////////////////////////////////////////////
	//  Stable streaming Sobel on the verified post-DDR RGB path.
	reg [10:0] sobel_x;
	reg [9:0]  sobel_y;
	...
	wire sobel_hs;
	wire cyber_mode;
	edge_mode_button u_edge_mode_button (
		.clk(clk_pixel), .rst_n(rstn_pixel),
		.key_n(key_mode_n), .frame_blank(!lcd_vs), .cyber_mode(cyber_mode)
	);
	// GitHub FPGA_Sobel algorithm port: weighted grayscale followed by
	// saturated |Gx|+|Gy| Sobel magnitude.  Camera, DDR3 and HDMI remain on
	// the verified Ti60F225 data path.
	github_sobel_stream_720p #(.WIDTH(1280), .HEIGHT(720)) u_sobel_stream (
		.clk(clk_pixel),
		.rst_n(rstn_pixel),
		.cyber_mode(cyber_mode),
