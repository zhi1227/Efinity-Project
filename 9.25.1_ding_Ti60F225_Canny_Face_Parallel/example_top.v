...
(
	...
	input             key_mode_n,            // Core-board K1, active low; replaces unused parallel LCD DE.
 input             key_freeze_n,          // Core-board K2 / M2, replaces unused LCD clock output.
	...
	wire 			w_wframe_vsync; 
	wire 	[7:0] 	w_axi_tp; 
 wire freeze_active_axi,freeze_press;
 reg freeze_request;
 (* async_reg="true" *) reg freeze_meta_pixel,freeze_pixel;
 button_press u_freeze_button(.clk(clk_pixel),.rst_n(rstn_pixel),.key_n(key_freeze_n),.pulse(freeze_press));
 always @(posedge clk_pixel or negedge rstn_pixel)
 if(!rstn_pixel) begin freeze_request<=0;freeze_meta_pixel<=0;freeze_pixel<=0;end
 else begin
   if(freeze_press) freeze_request<=~freeze_request;
   freeze_meta_pixel<=freeze_active_axi;freeze_pixel<=freeze_meta_pixel;
 end
  ...
  .freeze_request(freeze_request),.freeze_active(freeze_active_axi),
		.tp_o 		(w_axi_tp)
	);
 ...
 wire [23:0] face_overlay_rgb; wire face_overlay_de,face_overlay_vs,face_overlay_hs;
 wire [3:0] display_mode; wire [11:0] threshold_low,threshold_high;wire face_overrun;
 uart_params u_params(clk_pixel,rstn_pixel,uart_rx_i,display_mode,threshold_low,threshold_high,key_mode_n);
 assign uart_tx_o=1'b1;
 assign led_o[4]=face_overrun;
 // lcd_driver DATA_WIDTH=48: the original 24-bit RGB occupies the low half.
 parallel_video #(.HUD_ENABLE(1),.CANNY_GAUSSIAN(1)) u_parallel(clk_pixel,rstn_pixel,{lcd_red,lcd_green,lcd_blue},
 lcd_de,lcd_vs,lcd_hs,display_mode,threshold_low,threshold_high,
 face_overlay_rgb,face_overlay_de,face_overlay_vs,face_overlay_hs,face_overrun,freeze_pixel);
