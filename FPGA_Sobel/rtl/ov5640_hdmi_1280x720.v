...
always @(posedge clk_74m or negedge rst_n)
begin
    if(rst_n == 1'd0)
        rd_data_flag <= 1'd0;
    else
...
(
    .tft_clk    (clk_74m    ),   //TFT像素时钟
    .tft_rst    (rst_n     ),   //TFT复位信号，低电平有效
    .ip_flag    (rd_data_flag     ),   //图像数据有效标志
    .ip_data    (vga_rgb    ),   //输入图像数据

    .op_flag    (op_flag     ),   //Sobel处理后图像数据有效标志
    .op_data    (op_data     )    //Sobel处理后图像数据
);

key_filter key_filter_inst
(
    .sys_clk    (clk_74m    ),   //系统时钟
    .sys_rst_n  (rst_n     ),   //系统复位，低电平有效
    .key_input  (key_input ),   //按键输入

    .key_flag   (key_flag  )    //按键滤波后的输出
);
...
(
    ...
    .pix_data     (op_data   ),   //待显示数据
    .origin_data ({vga_rgb[15:11], 3'd0, vga_rgb[10:5], 2'd0,
