...
(
	...
	////////////////////////////////////////////////////////////////
	//  Face candidate detection and bounding-box overlay.
	//  This is the senior project's traditional vision pipeline, connected
	//  after the official OV5640 -> DDR3 -> RGB display path.
	reg [10:0] pixel_x_reg;
	reg [9:0]  pixel_y_reg;
	...
	always @(posedge clk_pixel or negedge rstn_pixel) begin
		if (!rstn_pixel) begin
			...
			if (pixel_x_reg == 11'd1279) begin
				pixel_x_reg <= 11'd0;
				pixel_y_reg <= pixel_y_reg + 1'b1;
			end else begin
				pixel_x_reg <= pixel_x_reg + 1'b1;
			end
		end
	end
	...
	face_reader_720p u_skin_detector (
		.image_in_R(lcd_red),
		.image_in_G(lcd_green),
		.image_in_B(lcd_blue),
		.pixel_x(pixel_x_reg),
		.pixel_y(pixel_y_reg),
		.pixel_valid(lcd_de),
	...
	low_pass_realtime u_low_pass_filter (
	...
	wire frame_start = lcd_vs & ~lcd_vs_d;
	wire frame_end   = ~lcd_vs & lcd_vs_d;
	...
	wire [9:0]  ccl_max_y;
	wire        ccl_bbox_valid;
	wire [21:0] ccl_area_pix;
	wire [3:0]  ccl_led_unused;

	streaming_connected_components #(
		.Wb(11), .Hb(10), .Nb(10), .MAX_BLOBS(64),
		.MIN_W(20), .MIN_H(20), .MIN_AREA(400), .AREA_W(22),
		...
		.blob_count(ccl_count), .raddr(ccl_raddr_pf),
		.r_min_x(ccl_min_x), .r_max_x(ccl_max_x),
		.r_min_y(ccl_min_y), .r_max_y(ccl_max_y),
		.r_valid(ccl_bbox_valid), .r_area_pix(ccl_area_pix),
		.break_en_o(break_en_for_next), .led(ccl_led_unused)
	);

	wire [5:0]  filt_count;
	wire [5:0]  filt_raddr;
	wire [10:0] filt_min_x;
	wire [10:0] filt_max_x;
	wire [9:0]  filt_min_y;
	wire [9:0]  filt_max_y;
	wire        filt_valid;

	// Eight simultaneous boxes are enough for the first stable reproduction.
	// The senior version used 32 and created a very long single-cycle min-search.
	bbox_filter_720p #(
		.WIDTH(1280), .HEIGHT(720), .MAX_IN_BLOBS(64), .MAX_OUT_BLOBS(8)
	) u_bbox_filter_720p (
		.clk(clk_pixel), .rst_n(rstn_pixel), .vs_in(lcd_vs),
		.in_blob_count(ccl_count), .in_addr_out(ccl_raddr_pf),
		.in_bbox_min_x(ccl_min_x), .in_bbox_max_x(ccl_max_x),
		.in_bbox_min_y(ccl_min_y), .in_bbox_max_y(ccl_max_y),
		.in_area_pix(ccl_area_pix), .in_bbox_valid(ccl_bbox_valid),
		.blob_count(filt_count), .addr_in(filt_raddr),
		.bbox_min_x(filt_min_x), .bbox_max_x(filt_max_x),
		.bbox_min_y(filt_min_y), .bbox_max_y(filt_max_y),
		.bbox_valid(filt_valid), .fetching_active()
	);
	...
	wire overlay_vs;
	wire overlay_hs;
	bbox_overlay #(.WIDTH(1280), .HEIGHT(720), .MAX_BLOBS(8)) u_bbox_overlay (
		.clk(clk_pixel), .rst_n(rstn_pixel),
		.rgb_in({lcd_red, lcd_green, lcd_blue}), .de_in(lcd_de),
		.vs_in(lcd_vs), .hs_in(lcd_hs),
		.x_in(pixel_x_reg), .y_in(pixel_y_reg),
		.blob_count(filt_count), .addr_out(filt_raddr),
		.bbox_min_x(filt_min_x), .bbox_max_x(filt_max_x),
		.bbox_min_y(filt_min_y), .bbox_max_y(filt_max_y),
